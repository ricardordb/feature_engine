...
class MedianEncoder(CategoricalInitMixin, CategoricalMethodsMixin):
    """
    The MedianEncoder() replaces categories by the median value of the target for each
    category.

    For example in the variable colour, if the median of the target for blue, red
    and grey is 2, 1 and 3 respectively, blue is replaced by 2, red by 1
    and grey by 3.

    For rare categories, i.e., those with few observations, the median target value
    might be less reliable. To mitigate poor estimates returned for rare categories,
    the median target value can be determined as a mixture of the target median value for
    the entire data set (also called the prior) and the median target value for the
    ...
    More details in the :ref:`User Guide <median_encoder>`.
    ...
    smoothing: int, float, str, default=0.0
        ...
        median target value per category is returned without modification. If 'auto' then
    ...
    ----------
    encoder_dict_:
        Dictionary with the target median value per category per variable.
    ...
    -------
    fit:
        Learn the target median value per category, per variable.
    ...
    >>> import pandas as pd
    >>> from feature_engine.encoding import MedianEncoder
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        """
        Learn the median value of the target for each category of the variable.
        ...
        for var in self.variables_:
            ...
            self.encoder_dict_[var] = (
                _lambda * y.groupby(X[var]).median() + (1.0 - _lambda) * y_prior
            ).to_dict()
